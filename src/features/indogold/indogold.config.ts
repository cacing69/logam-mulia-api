...
export const indogoldConfig: ScrapingConfig<'sell' | 'buy' | 'type' | 'info'> = {
	engine: 'cheerio',
	currency: 'IDR',
	...
	items: [
		{
			...
			postProcess: (rawData) => {
				...
				const buyMatch = text.match(/Harga Beli[\s\S]*?Rp\s*([\d,\.]+)/i);
				const sellMatch = text.match(/Harga Jual[\s\S]*?Rp\s*([\d,\.]+)/i);
