...
import { scrapeLakuemasForFetchOrCache } from './lakuemas.scrape';

const app = new Hono<{ Bindings: Bindings }>();

app.get('/', async (c) => {
	const refresh = c.req.query('refresh') === 'true';
	const result = await fetchOrCache(c.env, lakuemasConfig.name, { refresh }, () =>
		scrapeLakuemasForFetchOrCache(c.env, defaultScrapingOptions),
	);
