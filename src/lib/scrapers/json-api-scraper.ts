...
import type { JsonApiConfig } from '../types/config.types';
import type {
	JsonApiItem,
	JsonFieldMapping,
	FetchOptions,
...
export class JsonApiScraper<T extends Record<string, string> = Record<string, string>> {
	private config: JsonApiConfig;
	private source: string;

	constructor(source: string, config: JsonApiConfig) {
		this.source = source;
		this.config = config;
	}

	private async fetchJson(url: string, options?: FetchOptions): Promise<unknown> {
	...
	private mapSelector(payload: unknown, selector: JsonFieldMapping): Record<string, string> {
		...
		for (const [key, value] of Object.entries(selector)) {
			if (typeof value === 'string') {
				...
				const extracted = getByPath(payload, value);
				rawData[key] = extracted === undefined ? value : toStringValue(extracted);
	...
	async scrape<TOutput = T>(
		...
		try {
			...
			const selectorItems: JsonApiItem[] =
				this.config.items ??
...
/** @deprecated Use JsonApiScraper */ export const AxiosScraper = JsonApiScraper;
